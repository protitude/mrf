...
$art_style = '';
$art_head = '';
function RJF_process_html(&$variables) {
	...
	$themePath = RJF_art_get_full_path_to_theme();
...
function RJF_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];

  if (!empty($breadcrumb)) {
    // Provide a navigational heading to give context for breadcrumb links to
    // screen-reader users. Make the heading invisible with .element-invisible.
    $output = '<h2 class="element-invisible rjf-postheader">' . t('You are here') . '</h2>';

    $output .= '<div class="breadcrumb rjf-postcontent">' . implode(' | ', $breadcrumb) . '</div>';
    return $output;
  }
}
 ...
 * @ingroup themeable
 */
function RJF_button($variables) {
  $element = $variables['element'];
  $element['#attributes']['type'] = 'submit';
  element_set_attributes($element, array('id', 'name', 'value'));

  $element['#attributes']['class'][] = 'form-' . $element['#button_type'] . ' rjf-button';
 ...
 */
function RJF_preprocess_page(&$vars) {
  global $art_style, $art_head;
 ...
 * @ingroup themeable
 */
function RJF_menu_local_task($variables) {
  ...
  //added rjf-class
  $link['localized_options']['attributes']['class'] = array('rjf-button');
 ...
 */
function RJF_feed_icon($variables) {
  $text = t('Subscribe to @feed-title', array('@feed-title' => $variables['title']));
  return l(NULL, $variables['url'], array('html' => TRUE, 'attributes' => array('class' => array('feed-icon', 'rjf-rss-tag-icon'), 'title' => $text)));
}
 ...
 * @ingroup themeable
 */
function RJF_node_preview($variables) {
  $node = $variables['node'];
 ...
 */
function RJF_art_links_woker_D7($content) {
  $result = '';
  ...
  foreach (array_keys($content['links']) as $name) {
	...
	if (isset($content['links'][$name]['#links'])) {
	  ...
	  if (is_array($links)) {
		$output = RJF_art_links_html_output_D7($links);
		if (!empty($output)) {
...
function RJF_art_terms_D7($content) {
	$result = NULL;
...
function RJF_art_links_html_output_D7($links) {
	$output = '';
...
*/
function RJF_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'search_block_form' || $form_id == 'search_form') {
    $form['#attributes'] = array('class' => array('rjf-search')); // Add Artisteer class
    $form['actions']['submit']['#attributes'] = array('class' => array('rjf-search-button')); // Add Artisteer class
  }
}

function RJF_art_label($variables) {
  $element = $variables['element'];
  ...
  $art_class = $element['#type'] == 'checkbox' ? ' rjf-checkbox' : ($element['#type'] == 'radio' ? ' rjf-radiobutton' : '');
...
function RJF_form_element_label($variables) {
  $art_label = RJF_art_label($variables);
...
function RJF_form_element($variables) {
  $element = &$variables['element'];
  // This is also used in the installer, pre-database setup.
  $t = get_t();
...
function RJF_pager($variables) {
  ...
  if ($pager_total[$element] > 1) {
    ...
    return '<h2 class="element-invisible">' . t('Pages') . '</h2>' . RJF_art_item_list(array(
      'items' => $items,
      'attributes' => array('class' => array('rjf-pager')),
    ));
  }
}

function RJF_art_item_list($variables) {
  ...
  if (!empty($items)) {
    ...
    foreach ($items as $i => $item) {
      ...
      if (count($children) > 0) {
        // Render nested list.
        $data .= RJF_art_item_list(array('items' => $children, 'title' => NULL, 'type' => NULL, 'attributes' => $attributes));
      }
      if ($i == 0) {
